...
const LoginForm = () =>{
  ...
  const varifyOtp = (e) =>{
    ...
    if(OTP.length ===6){
      if (OTP===authOtp.otp){
        ...
        axios.post('http://localhost:8081/verifyotp',authOtp.data) 
    .then(res=>{
  ...
  const onLoginSuccess = (jwtToken,userId) =>{
    ...
    navigate('/home',{ replace: true }) //it won't go back
    //navigate('/')
  }
  ...
  const onSignIn = (e) =>{
    ...
    e.preventDefault()
    axios.post('http://localhost:8081/login',signInData) 
    .then(res=>{
  ...
  const onSignUp = (e) =>{
    ...
    axios.post('http://localhost:8081/data',signUpData) 
    .then(res=>{
  ...
  const displayotp = (e) =>{
    ...
    axios.post('http://localhost:8081/getotp',{'mobile':showotp.mobile}) 
    .then(res=>{
